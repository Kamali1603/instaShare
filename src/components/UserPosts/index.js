...
class UserPosts extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const {searchPosts} = nextProps
    if (searchPosts !== prevState.searchPosts) {
      return {searchPosts}
    }
    return null
  }
  ...
  getUserPosts = async (searchQuery = '') => {
    this.setState({apiStatus: apiStatusConstants.inProgress})
    const jwtToken = Cookies.get('jwt_token')
    const apiUrl = searchQuery
      ? `https://apis.ccbp.in/insta-share/posts?search=${searchQuery}`
  ...
  renderFailureView = () => {
    ...
    return (
      <div className="failure-view-container">
        ...
        <button
          onClick={() => this.getUserPosts(searchInput)}
          type="submit"
